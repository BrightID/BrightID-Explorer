...
def run():
    ...
    scores = {v: {'name': 'Bitu', 'user': v, 'linksNum': 0, 'confirmedScore': 0, 'score': 0, 'directReports': {
    ...
    for e in main_component.edges:
        # skip the edges that the source or target is not eligible
        ...
        if other_side_level in ['already known', 'recovery']:
            ...
            scores[e[0]]['score'] -= config.DIRECT_PENALTY
    ...
    for e in main_component.edges:
        ...
        if len(scores[e[1]]['directReports']) > 0:
            ...
            scores[e[0]]['score'] -= config.INDIRECT_PENALTY * \
                len(scores[e[1]]['directReports'])

    for n in scores:
        if n in supervised_verifieds:
            supervised_verifieds[n]['score'] = scores[n]['score']
        elif n not in supervised_verifieds and scores[n]['score'] > 0:
            supervised_verifieds[n] = scores[n]
    for n in supervised_verifieds:
        if n not in scores:
            supervised_verifieds[n]['score'] = 0

    res = json.dumps(list(supervised_verifieds.values()))
    with open(config.BITU_FILE, 'w') as f:
        f.write(res)
    upload(config.BITU_FILE)
